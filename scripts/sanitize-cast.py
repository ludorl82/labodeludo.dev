#!/usr/bin/env python3
"""Assainit une PRISE RÉELLE asciinema avant publication.

Usage : python3 scripts/sanitize-cast.py <prise.cast> <public/casts/x.cast>

Coupe ce qui précède le premier `clear`, préfixe le préambule maison, substitue les
noms d'hôte dans les SORTIES (colonnes kubectl réalignées), puis REJOUE le cast
dans un émulateur (pyte) et refuse (rc 1) si un vrai nom apparaît à l'écran à un
moment quelconque — le flux brut ne suffit pas, zsh émet les lettres une à une.
Le shell de prise doit être minimal (invite sans utilisateur, sans suggestion
automatique) : une substitution dans une ligne en cours d'édition casse le curseur.
"""
import json, re, sys
...
SUBS = [("bob","gpu-01"), ("stella","gaming-01")]
def sub(s):
    for a,b in SUBS:
    ...
    return s
ev = [[round(e[0]-t0+3.4,3), e[1], sub(e[2])] for e in ev]
...
pre = ("\x1b[H\x1b[2J\r\n"
       ...
       f"     {DIM}avec asciinema dans une session tmux dédiée, telle quelle : noms d'hôte et{R}\r\n"
       f"     {DIM}d'utilisateur substitués, pauses plafonnées à 2 s.  labodeludo.dev/casts/{R}\r\n")
ev = [[0.0,"o",pre]] + ev
...
BAD = ["ludorl82","bob","stella","worker","coquille","tptpt","172.16.","10.10.","labodeludo.dev/api"]
seen = {}
for e in ev:
    st.feed(e[2])
    text = "\n".join(scr.display)
