...
The NAS — let's call it **Grenier** — is a small two-bay QNAP box, ARM processor, 2 GB of RAM. Nothing extravagant. It holds the household media library on an 8 TB hard drive, and it also serves as NFS storage for the k3s cluster.

## The diagnosis: 670 MB keeping 7 TB awake

First thing to do before buying or unplugging anything: look at what is actually on those shares.
...
But size was not the problem — the write pattern was. Those 670 MB are the data files of Postgres, Mongo, Loki, a few SQLite databases. Databases, in other words, constantly writing small blocks to scattered locations. That is precisely the I/O profile a hard drive hates: the head moves nonstop, and above all the drive never gets to spin down.
...
The filesystem showed up as `UNKNOWN`. No shared folders on it, so nothing visible in the file browser. The classic signature of a disk that came from another system and dragged its old partition table along with it.

437 GB of something. No way to tell what.

This is the kind of moment where you stop. A disk described as "spare" that contains half a terabyte of data does not get formatted on an impression. I asked before erasing. The answer came back — it really was old stock — and only then did I delete the volume.
...
The new volume was created as a static volume — better performance, less memory overhead, which matters on an ARM box with 2 GB of RAM. Without encryption, too, and that is deliberate: an encrypted volume demands a passphrase at every boot, which means that after a power cut the NFS would not come back until somebody walked over and typed a password. For storage that has to return on its own, that is a no.
...
The naive approach would be to create a `k8s-ssd` share, copy the data over, then go edit those three references. Except that modifying a persistent volume bound to a claim means unbinding, recreating, rebinding — for fifteen or so volumes. A lot of handling, a lot of chances to get it wrong.
...
## Two QNAP traps that don't announce themselves

This is where the evening got longer.

**Trap number one: renaming a shared folder wipes its NFS permissions.** After the rename, `k8s-hdd` simply stopped appearing in the list of exports. The "access rights" box had unchecked itself. Nothing in the interface mentions it, and you only find out at the moment the mount fails.

The habit worth forming: after any share manipulation, run `showmount -e` against the server. It is the only source of truth about what is actually exported — the web interface tells you what it believes, `showmount` tells you what is.
...
One `kill -9`, twenty seconds of waiting, and the next mount worked first try.
...
The transfer itself is the least interesting part: `rsync` in archive mode, with extended attributes and numeric IDs. Six thousand nine hundred and twenty-two files, 693 MB, roughly two minutes.
...
It will be deleted once everything has run for a few days. Not before. A migration is not finished the evening it works — it is finished the day you remove the safety net, and that day comes later than you think.
