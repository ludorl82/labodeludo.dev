...
That is exactly what the nightly drift checks measure, and they existed before this experiment. Three of them query their respective provider and compare the real state against the plan computed from the repository. The fourth layer no longer needs one: the cluster runs on GitOps — an agent permanently compares its real state against the repository and corrects the gaps itself ([how I got there](/en/blog/laisser-le-pipeline-appuyer-sur-apply/)). It therefore asks the same question continuously rather than once a night.
