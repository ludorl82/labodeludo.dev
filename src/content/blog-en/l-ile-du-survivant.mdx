...
> -   **Concept**: a "survivor island", the router, the WAN modem and a single Raspberry Pi grouped on the least-loaded UPS in the house (about 15 W, hours of runtime), the Pi plugged directly into a hardware switch port on the router. The island routes, alerts and keeps remote access alive while everything else shuts down cleanly.
...
> -   **Heartbeat semantics**: the dead-man switch's "the house is alive" signal (S3, then Lambda, then email) is emitted from the island, precisely because it is the last thing to die. A stale heartbeat then means "nothing can communicate anymore", the only case where an external email adds information.
...
- **A firmware that waits for a thumb.** After a complete on-battery power-off, the rack UPS does not re-energize its outlets when mains return. No exposed setting, no menu. That discovery is what pivoted the design toward "keep the outlets hot and IPMI latch" rather than "cut power and let `always-on` handle it".
