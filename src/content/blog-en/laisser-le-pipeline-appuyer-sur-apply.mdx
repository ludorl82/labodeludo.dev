...
pubDate: 2026-07-31
description: "The whole lab was already described as code, but every apply still went through my hands. We closed the loop in one session: GitHub Actions with OIDC for the cloud, Argo CD for the cluster, comin for NixOS. The interesting part isn't the plumbing: it's what the gate refuses, GitHub's OIDC surprise, and the two Raspberry Pis that choked on a test-only dependency."
tags: ["DevOps", "ludo"]
...
> -   **The surprises**: GitHub's *immutable OIDC subjects* break classic trust policies; two 4 GB Pi 5s choked on a dependency of a Markdown parser's test suite.
