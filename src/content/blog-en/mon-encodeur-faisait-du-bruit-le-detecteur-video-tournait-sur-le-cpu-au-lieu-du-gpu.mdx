...
> - **Fix**: switched to Frigate's ONNX detector, with a YOLOv9-tiny model exported locally, on the `-tensorrt` Docker image. The detector now runs on the GPU. CPU back down to about 27%.
...
- `nvidia-smi` and `ps aux --sort=-%cpu` remain the fastest pair of commands to tell "it's the GPU" from "it's the CPU" apart when a machine is making more noise than usual.
- A config error can live for months in the logs without ever crashing anything. It just silently disables a feature (here, the motion mask) until someone goes reading the logs for a completely different reason.

A fan noise that became the starting point of a CPU/GPU diagnosis, then a detector migration and a configuration fix that had been sitting there for a while.
