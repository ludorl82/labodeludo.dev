...
> **Update, September 20, 2026.** The reconstruction above show the session; the mechanism, him, you can replay. Here he is for real, on a copy of the repository: a clean pass, a fictional domain planted in a DNS zone, the refusal, a one-line fix, and the proof in the sanitized tree. Keystrokes sent by a script, real output.

<AsciiCast
  src="/casts/iac-sanitize-gate-real.cast"
  frame="none"
  poster="npt:0:38"
  caption="The gate for real: the script on top, git diff --stat watching at the bottom. One forgotten domain is enough for it to refuse, one line is enough for it to pass again."
