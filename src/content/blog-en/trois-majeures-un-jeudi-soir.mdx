...
So we encoded the lessons instead of just remembering them: major versions now wait for **explicit approval** on a dashboard before they even become PRs; software that is several majors behind will get them **one at a time**; database majors **never come from a robot again** — that's a planned maintenance with a runbook, not a Sunday surprise; and Kuma's probe now has a one-hour startup window, because a migration deserves respect.
