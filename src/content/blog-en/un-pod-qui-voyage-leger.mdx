...
> -   **The result**: a pod with no volume. Evicted, rescheduled, ready on another node in 12 seconds, history intact, because the state lives in a versioned S3 bucket that did not even notice the drain.
...
> **Update, September 20, 2026.** The recording above is a reconstruction, and says so. This one is not. Six weeks later I redid the same eviction for real on the cluster, in a tmux session split in two and captured as is: on top I cordon the node and restart the deployment, at the bottom a `kubectl get pods -w` watches the old pod die and the new one come back on another node. 12 seconds, timed by the shell. The reconstruction's number was right; I just did not know it with that precision before. The machine names are the real ones, and only the pauses are capped at two seconds.

<AsciiCast
  src="/casts/stateless-move-real.cast"
  frame="none"
  poster="npt:0:39"
  caption="The real take: the cordon and the restart on top, the pod dying and coming back on another node at the bottom, 12 seconds on the shell's stopwatch."
  disclaimer="✔ Unlike the recording above, this is a real capture, made on September 20, 2026 with asciinema in a dedicated tmux session, with no editing. The machine names are the real ones; only pauses longer than two seconds are shortened."
/>
