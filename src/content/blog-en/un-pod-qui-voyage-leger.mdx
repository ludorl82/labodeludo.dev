...
<AsciiCast
  ...
  frame="none"
  poster="npt:0:42"
  caption="For real, six weeks after the migration: on top I cordon the node and restart the deployment, at the bottom a kubectl get pods -w watches the old pod die and the new one come back on another node, 12 seconds on the shell's stopwatch."
  disclaimer="✔ This is a real capture, made on September 20, 2026 with asciinema in a dedicated tmux session, with no editing. The machine names are the real ones, the prompt shows “ludo”, the site's short name, and only pauses longer than two seconds are shortened."
/>
...
> **Note, September 20, 2026.** This recording replaced the reconstruction that sat here since publication. It remains on [its own page](/casts/deplacer-un-pod-stateless/), with its disclaimer, and it shows one thing the real take can no longer show: the "before", the pod riveted to its disk, staying Pending in front of eight free nodes. The number, though, was right.
