...
> **Update, September 20, 2026.** The recording above is a reconstruction, and says so. This one is not. Six weeks later I redid the same eviction for real on the cluster, in a dedicated tmux session captured as is: the pod runs on one node, I cordon it, I restart the deployment, and I watch the pod come back on another node. 12.2 seconds, timed by the shell. The reconstruction's number was right; I just did not know it with that precision before. Only the hostnames are substituted, and pauses are capped at two seconds.

<AsciiCast
  src="/casts/stateless-move-real.cast"
  poster="npt:2:07"
  caption="The real take: cordon the node, restart the deployment, and the pod is ready on another node in 12.2 seconds, with the shell's stopwatch as witness."
  disclaimer="✔ Unlike the recording above, this is a real capture, made on September 20, 2026 with asciinema in a dedicated tmux session, with no editing. Hostnames are substituted and pauses longer than two seconds are shortened; nothing else was touched."
/>
