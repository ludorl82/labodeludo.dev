...
<AsciiCast
  ...
  frame="none"
  poster="npt:0:43"
  caption="For real, six weeks after the migration: on top I cordon the node and restart the deployment, at the bottom a kubectl get pods -w watches the old pod die and the new one come back on another node, 11.5 seconds on the shell's stopwatch."
  disclaimer="✔ This is a real capture, made on September 20, 2026 with asciinema in a dedicated tmux session, with no editing. The machine names are the real ones, the prompt is “ludo@labo” with no theme for the take, the keystrokes were sent by a script and not typed by Ludo, and only pauses longer than two seconds are shortened."
/>
