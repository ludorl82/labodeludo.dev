...
pubDate: 2026-07-16
description: "Comment je fais tourner Claude Code dans une session tmux persistante sur mon bastion conteneurisé, comment le mode automatique enchaîne de longues sessions à partir d'un petit prompt, comment une instance headless du même agent exécute mes sauvegardes et mes mises à jour système sans supervision — et pourquoi l'abonnement Claude Pro me coûte 40 $/mois plutôt que des centaines en jetons API."
...
> -   **Correction (2026-08-01, corrigée à son tour le 2026-08-02)** : les sauvegardes hebdomadaires sont des scripts planifiés conventionnels ; les mises à jour, elles, sont un hybride — script déterministe pour les serveurs, instance headless de Claude Code pour les appareils. Voir les notes dans la section concernée.
...
> **Correction (2026-08-01)** : cette section décrivait les sauvegardes et les mises à jour système hebdomadaires comme exécutées par une instance headless de Claude Code. Ce n'est pas exact : ces deux routines roulent comme des scripts planifiés tout à fait conventionnels sur le bastion — déterministes, sans agent dans la boucle d'exécution. Claude Code a conçu, écrit et fait évoluer ces scripts (et le mode headless décrit ici existe bel et bien), mais ce n'est pas lui qui exécute les sauvegardes et les mises à jour chaque semaine. Je laisse le texte original tel quel, avec cette note, plutôt que de le réécrire en douce.
>
> **Correction de la correction (2026-08-02)** : en retravaillant la routine cette semaine, j'ai découvert que ma correction allait trop loin. Les sauvegardes sont bel et bien des scripts conventionnels, et la mise à jour des serveurs aussi — déterministe, sans agent. Mais le volet « appareils » de la même routine hebdomadaire — le NAS, la domotique, le contrôleur Wi-Fi, et depuis cette semaine le pare-feu — est exécuté par une instance headless de Claude Code, lancée par le script avec un runbook intégré. Sur ce point, l'article original disait vrai. Ce paragraphe aura donc été corrigé dans les deux directions ; je m'arrête ici.
...
<ConversationExcerpt
  ...
  caption="Noms d'infrastructure sanitisés."
...
C'est ce mode-là qui a rendu possible la majorité du travail derrière [ma grappe k3s](/blog/deployer-un-cluster-k3s-avec-claude-code/) : un prompt de départ assez court, puis des heures de travail continu — diagnostic, correctifs, validations — sans que j'aie à valider chaque commande une à une.
...
![Capture d'écran d'une session Claude Code dans le conteneur console sur le bastion, montrant la barre de statut tmux avec le mode automatique et le contrôle à distance actifs](/images/blog/claude-code-tmux-auto-mode-screenshot-1400x669.png)
...
## Prochaine exploration : Claude Cowork et Claude dans Chrome

Ce même abonnement Claude Pro inclut aussi Claude Cowork, que je n'ai pas encore essayé — c'est sur ma liste pour un prochain article. J'utilise par contre déjà Claude dans Chrome, l'extension qui laisse l'agent naviguer et agir directement dans le navigateur, et je la trouve pas mal utile. J'ai envie d'en parler bientôt, alors : à suivre.
...
-   Le même agent qui m'aide en session interactive peut tourner sans supervision, sur un horaire — les sauvegardes et la mise à jour des serveurs restent des scripts conventionnels, mais la mise à jour des appareils est bel et bien pilotée par une instance headless (voir les corrections plus haut).
