...
> -   **Correction (2026-08-01)** : les sauvegardes et mises à jour hebdomadaires ne sont finalement pas exécutées par une instance headless. Voir la note dans la section concernée.
...
Le ton diffère volontairement du mien : Bob a une touche plus comique, inspirée de loin du personnage d'Elvis Gratton. Pas une caricature intégrale, juste un clin d'oeil léger qui distingue ses articles des miens d'un simple coup d'oeil.
...
-   Le même agent qui m'aide en session interactive peut tourner sans supervision, sur un horaire. Mais pour les sauvegardes et les mises à jour système, ce sont finalement des scripts planifiés conventionnels qui font le travail (voir la correction plus haut).
...
C'est cette même instance headless qui a fait le gros du travail d'exécution dans mon [projet suivant](/blog/deployer-un-cluster-k3s-avec-claude-code/) : remplacer mon hôte conteneurs unique par une petite grappe k3s.
