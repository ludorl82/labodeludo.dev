...
La différence avec un navigateur sans interface, c'est que ça roule dans **ma** session, dans **mon** navigateur, avec mes onglets ouverts. Chrome affiche d'ailleurs une bannière bien visible pendant toute la durée — on la voit dans l'image d'en-tête de cet article. Impossible d'oublier que l'agent est aux commandes.
...
_Note sur l'image d'en-tête : c'est une vraie capture de la session, avec le nom d'hôte interne remplacé par un domaine d'exemple._
