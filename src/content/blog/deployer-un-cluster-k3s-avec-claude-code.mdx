...
pubDate: 2026-07-18
description: "Comment j'ai remplacé l'hôte conteneurs unique de mon labo par une petite grappe k3s de six nœuds — un plan de contrôle dans le nuage, cinq nœuds à la maison — en travaillant en session avec Claude Code, jusqu'aux sauvegardes chiffrées qui tournent maintenant toutes seules, la nuit."
...
> -   **Résultat** : le pipeline de déploiement de ce site roule maintenant sur la grappe plutôt que sur une seule machine, et Claude Code orchestre désormais, chaque semaine, la nuit, les sauvegardes chiffrées de chaque nœud depuis le bastion.

Cette semaine, j'ai pris l'hôte conteneurs unique qui faisait tourner mon labo — dont ce site — et je l'ai transformé en une vraie petite grappe Kubernetes. Rien de spectaculaire à l'œil nu une fois le travail terminé, mais en dessous, c'est un tout autre genre d'animal. Comme pour la plupart des projets d'infrastructure ici, j'ai fait le gros du travail en session avec Claude Code : je pose le contexte et les décisions d'architecture, il exécute, diagnostique et documente au fur et à mesure.
...
La grappe tourne, le site que vous lisez roule dessus, et les sauvegardes se font pendant que tout le monde dort. Pas mal pour une semaine de travail. — Ludo
