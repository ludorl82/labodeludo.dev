...
pubDate: 2026-07-18
description: "Remplacer l'hôte conteneurs unique du labo par une petite grappe k3s de six nœuds — un plan de contrôle dans le nuage, cinq nœuds à la maison — en travaillant de bout en bout avec Claude Code, jusqu'au déménagement du pipeline de déploiement de ce site sur la nouvelle grappe."
...
> -   **Solution** : bâtir une petite grappe **k3s** — une distribution Kubernetes allégée, plus adaptée qu'un Kubernetes « classique » à six machines dont deux sont des Raspberry Pi — de six nœuds, répartie entre un plan de contrôle dans le nuage et cinq nœuds à la maison, en travaillant de bout en bout avec Claude Code comme copilote pour le déploiement.
...
Cette semaine, j'ai pris l'hôte conteneurs unique qui faisait tourner mon labo — dont ce site — et je l'ai transformé en une vraie petite grappe Kubernetes. Comme pour la plupart des projets d'infrastructure ici, j'ai fait le gros du travail en session avec Claude Code : je pose le contexte et les décisions d'architecture, il exécute, diagnostique et documente au fur et à mesure. Ce n'est d'ailleurs pas la première fois que je parle de la manière dont je travaille avec lui — j'ai décrit récemment comment [une instance headless de Claude Code](/blog/claude-code-headless-bastion/), tournant sans supervision depuis mon bastion, s'occupe déjà des sauvegardes et des mises à jour système de tout le labo. C'est cette même instance qui a exécuté une bonne partie de ce qui suit.
...
Pour donner une idée concrète de comment ça se passe en pratique, voici un extrait de la session sur le tout premier Raspberry Pi :
