...
Des trois machines virtuelles, deux tournent sur des machines de calcul Linux déjà en place dans le labo, la troisième sur une machine Windows avec Hyper-V. C'est une bonne occasion de voir la polyvalence de Claude Code à l'œuvre — la même session, le même agent, mais un niveau d'autonomie très différent selon la plateforme.

Sur les deux machines Linux, Claude Code a fait la création des VM de bout en bout sans que j'aie à toucher à quoi que ce soit : définir la VM (CPU, mémoire, disque), l'attacher au bon réseau, démarrer l'image de base, s'y connecter en SSH, et enchaîner directement sur l'installation de k3s. Zéro intervention manuelle de ma part.
