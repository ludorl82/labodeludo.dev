...
Le coffre-fort de mots de passe est accessible depuis n'importe où via WebDAV, sans VPN. C'est un choix qui simplifie la vie au quotidien mais qui déplace tout le poids de la sécurité sur l'authentification de la porte d'entrée. Cette porte tournait encore avec un hash `$apr1$` (une variante MD5 du bon vieux `htpasswd` Apache) et n'avait aucune protection contre la force brute. Quelqu'un qui tombe sur l'URL pouvait essayer autant de mots de passe qu'il voulait, aussi vite qu'il voulait.
...
Un hash plus solide, une porte qui ralentit les acharnés, et un forfait qui, lui, n'a pas résisté aussi bien que le coffre-fort.
