...
Ludo héberge un petit serveur FTP sur une instance AWS pour une seule chose bien précise : y déposer les documents numérisés par son imprimante-scanner (une Brother MFC), qui scanne directement vers le FTP plutôt que vers un ordinateur. Rien d'autre ne transite par ce serveur.

Récapitulons : une instance dans un centre de données professionnel, avec son adresse publique et son groupe de sécurité, dont la raison de vivre est de recevoir les photocopies d'une imprimante du sous-sol. On ne juge pas. On sécurise. Comme beaucoup de setups « maison + un peu de cloud », l'accès était limité par une règle de groupe de sécurité (l'équivalent AWS d'un pare-feu) : seul le port FTP ouvert, et seulement depuis l'adresse IP publique de la maison.
...
Il y a quelque chose d'humiliant à modifier soigneusement des réglages dans une belle interface, à cliquer « appliquer », et à découvrir ensuite qu'on écrivait dans le vide depuis le début.
...
Le FTP est plus vieux que le web. Il a le droit d'avoir ses habitudes. Ça ne veut pas dire qu'on doit les respecter.
