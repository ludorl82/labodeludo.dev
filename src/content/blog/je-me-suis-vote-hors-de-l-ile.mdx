...
Sauf que le vrai piège est un cran plus bas, et c'est celui-là que je n'avais pas vu venir : **éteindre un membre ne le retire pas de la liste des votants.** Son siège reste là, bien au chaud — la machine arrête simplement de répondre quand on l'appelle. Et la majorité se calcule sur le nombre de sièges, pas sur le nombre de machines encore debout.

Trois sièges, deux machines vivantes : c'est une majorité, ça passe. Deux sièges, une machine vivante : ce n'en est pas une, et tout se fige. C'est pour ça que la même séquence de commandes est passée sans broncher sur le premier nœud et a tout mis à terre sur le deuxième — ce n'est pas la commande qui avait changé, c'est le nombre de sièges autour de la table.

Et comme c'est justement l'annonce du départ qui libère le siège, et que cette annonce a besoin d'un cluster en état de marche pour être enregistrée, éteindre la machine en premier referme la seule porte de sortie. Résultat sur le deuxième nœud : silence complet, même du côté du nœud survivant.

<AsciiCast
  src="/casts/control-plane-consolidation.cast"
  poster="npt:0:08"
  caption="Le démontage au complet, condensé en un peu plus d'une minute. La même commande passe sans problème sur le premier nœud et met le cluster à terre sur le deuxième — et la commande qui répare ça se fait bloquer par le garde-fou de permissions au pire moment."
...
Il faut le faire : casser le quorum d'un cluster pendant l'opération dont le but explicite était de le rendre plus solide. J'avais lu la procédure. Je l'avais même comprise. Je l'ai simplement exécutée à l'envers. Solution : redémarrer le service sur la machine qu'on venait tout juste d'éteindre, le temps de rétablir assez de votants pour que tout le monde soit d'accord de nouveau, puis refaire l'opération dans le bon ordre — prévenir d'abord, éteindre ensuite. Cette fois, la leçon est écrite noir sur blanc dans la documentation du projet, pour ne pas la réapprendre une deuxième fois.
...
Notons quand même que j'ai passé une bonne partie de la matinée à chercher ce que j'avais brisé la veille, pour finalement trouver quelque chose que quelqu'un avait brisé des semaines plus tôt. Le soulagement était réel. La fierté, moins.
...
Deux votants éliminés, un seul survivant sur l'île, et personne pour contester le résultat. C'est la seule finale où le gagnant compte les votes tout seul.
