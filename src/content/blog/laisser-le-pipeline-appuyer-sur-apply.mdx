...
> -   **Les surprises** : les *sujets immuables* OIDC de GitHub cassent les politiques de confiance classiques ; deux Pi 5 de 4 Go se sont étouffés sur une dépendance de la suite de tests d'un parseur Markdown.
...
Pour le cluster et les hôtes NixOS, c'est l'inverse : rien n'y est joignable depuis l'internet, et c'est voulu. Plutôt que de percer un chemin pour que CI puisse pousser, c'est le labo qui tire — Argo CD roule dans le cluster, et [comin](https://github.com/nlewo/comin) roule sur chaque hôte NixOS, qui compile et bascule lui-même. Un compromis de mon compte GitHub pourrait proposer des commits, qui resteraient visibles et révocables ; il ne pourrait pas voler un kubeconfig, parce qu'il n'en existe aucun à l'extérieur du labo.

La surprise du jour n'a pas manqué : les premières exécutions ont toutes échoué sur `Not authorized to perform sts:AssumeRoleWithWebIdentity`. GitHub émet maintenant des **sujets immuables** (`repo:owner@id/repo@id:...`), et toute politique de confiance écrite d'après la documentation classique casse net. Les politiques acceptent désormais les deux formes ; comme les identifiants numériques sont immuables, rien n'est élargi au passage.
