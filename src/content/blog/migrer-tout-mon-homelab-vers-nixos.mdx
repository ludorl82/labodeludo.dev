...
>
> -   **Objectif** : convertir les neuf noeuds de mon cluster k3s, deux serveurs GPU, trois machines virtuelles, deux Raspberry Pi 5 et une instance EC2, vers NixOS, décrits dans un seul dépôt git.
...
En une seule journée, tout ça est devenu du passé. Les neuf noeuds de mon cluster k3s roulent maintenant NixOS, l'instance cloud comprise, et chacun est entièrement décrit dans un dépôt git que j'appelle `nixos-iac`.
...
**La restauration.** Restaurer un serveur Ubuntu, c'est restaurer une *image*, avec toute la sédimentation dedans, le bon comme le mauvais. Restaurer un noeud NixOS, c'est réexécuter sa description : la machine qui revient est propre, et identique à celle décrite dans git. Pendant la migration, on a d'ailleurs reformaté certaines machines plusieurs fois de suite pour corriger un détail, avec la désinvolture qu'on réserve d'habitude à un conteneur Docker.
...
outputs = { self, nixpkgs, disko, ... }@inputs: {
  ...
  # ... et ainsi de suite pour les neuf noeuds
};
```
...
Tout ce qui est commun vit dans `modules/`. L'exemple le plus payant : chaque noeud du cluster importe le même module d'agent k3s.

```nix
# modules/k3s-agent.nix, partagé par tous les noeuds
{ pkgs, ... }:
{
  services.k3s = {
    enable = true;
...
Écrit comme ça, ça a l'air lisse. Ça ne l'est jamais. Voici une conversion au complet, une des neuf, celle d'une machine virtuelle, condensée : la demande, un changement d'idée en cours de route, deux murs successifs, et la machine qui finit par revenir en NixOS.

<AsciiCast
  src="/casts/nixos-migration.cast"
  poster="npt:0:06"
  caption="Une conversion de noeud du début à la fin, condensée en moins de deux minutes. Le Secure Boot et l'initrd qui refuse de se décompresser sont les vrais murs de cette session."
...
Les deux murs de cette session valent la peine d'être nommés, parce qu'ils n'ont rien à voir avec NixOS : le Secure Boot de la machine virtuelle refusait de charger un noyau non signé (donc pas de kexec du tout), et une fois ça réglé, l'initrd de l'installateur, environ 430 Mo, ne se décompressait pas dans les 4 Go de RAM de la VM. Le diagnostic est passé par la console du superviseur, parce que la machine n'avait plus de réseau du tout.
...
Voir son unique noeud de plan de contrôle se faire reformater à distance, c'est le genre de moment où on relit la commande trois fois avant de peser sur Entrée. Le cluster est revenu comme si de rien n'était, et cette instance est maintenant décrite dans git au même titre que les huit autres noeuds. La frontière entre « mes machines » et « le cloud » n'existe plus dans le dépôt.
...
# épinglé sur le noeud cloud : une panne du réseau à la maison ne doit
# pas emporter l'outil censé m'en alerter. Laisser l'ordonnanceur le
# déplacer sur un noeud maison déferait silencieusement ce choix.
apiVersion: apps/v1
kind: Deployment
metadata:
  name: kuma
...
Les deux dépôts sont privés, parce qu'ils contiennent les vraies adresses, clés et certificats du labo. Mais un article sur « tout vit dans git » sonne un peu creux sans le git, alors j'ai publié un instantané assaini de chacun :
