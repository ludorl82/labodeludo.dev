---
title: "Tout ça pour un script bash : le jour où le pipeline de numérisation a perdu son image sur mesure"
pubDate: 2026-08-05
...
>
> -   **Point de départ** : SFTPGo en conteneur, avec une image Docker sur mesure (build multi-architecture, dépôt public, pipeline d'intégration continue) dont la seule raison d'être était un hook de post-téléversement — ImageMagick, rclone, l'interface AWS en ligne de commande et un client SSH, tous présents pour un script.
...
> -   **Le code** : assaini et public, étiquette `article/numeriseur-lambda` dans [aws-iac-public](https://github.com/ludorl82/aws-iac-public/blob/article/numeriseur-lambda/live/numeriseur.tf) (la Lambda, la règle EventBridge, le rôle et la file de messages morts) et [k3s-iac-public](https://github.com/ludorl82/k3s-iac-public/blob/article/numeriseur-lambda/numeriseur/deployment.yaml) (le pod sans état, les sondes, le rendu de configuration).
...
Ce que je n'aimais pas, c'est ce qu'il fallait entretenir pour que ça roule : une image Docker buildée maison, publiée sur un registre public, avec un pipeline d'intégration continue multi-architecture. Pour un service qui existe déjà, officiellement, en image publique et maintenue.

En ouvrant le capot, la raison tenait en un fichier : `post-upload.sh`, une soixantaine de lignes de bash. C'est pour lui que l'image embarquait ImageMagick, rclone, l'interface AWS en ligne de commande et un client SSH. Soixante lignes de script, et tout un pipeline de build autour.

<ScanPipelineDiagram context="article">
  Le pipeline après la bascule. Tout ce qui est à droite de la ligne pointillée
  remplace un script de soixante lignes qui roulait dans le conteneur : S3 reçoit
  l'écriture native de SFTPGo, EventBridge transforme la création de l'objet en
  invocation, et la Lambda livre vers Google Drive avec des identifiants qu'elle
  lit dans Secrets Manager. Le chemin d'échec — deux retries, puis la file de
  messages morts et son alarme — est la partie qui n'existait pas du tout avant.
</ScanPipelineDiagram>
...
Avant d'écrire quoi que ce soit, j'ai listé le contenu réel du bucket. Bonne habitude : c'est là que j'ai trouvé le premier vrai bogue, et il n'était pas dans mon mandat.
...
Une image officielle sans rien à construire. Aucun dépôt d'image à maintenir, aucun pipeline d'intégration continue multi-architecture, aucun registre public à surveiller. Un pod sans état : pas de volume, pas de sauvegarde nocturne, rien à perdre si le nœud disparaît. Des identifiants infonuagiques réduits au strict nécessaire, et les anciens — exposés deux fois dans des transcriptions — révoqués sur les deux comptes.
...
Les deux dépôts compagnons sont assainis — noms d'hôtes, adresses et
identifiants fictifs, commentaires réels — et étiquetés `article/numeriseur-lambda` :

| Fichier | Ce qu'il y a à voir |
|---|---|
| [`live/numeriseur.tf`](https://github.com/ludorl82/aws-iac-public/blob/article/numeriseur-lambda/live/numeriseur.tf) | La Lambda, la règle EventBridge, la file de messages morts et son alarme. Le rôle d'exécution est le passage intéressant : `GetObject` sans `PutObject`, avec le commentaire qui explique pourquoi la boucle devient inatteignable. |
| [`live/numeriseur/handler.py`](https://github.com/ludorl82/aws-iac-public/blob/article/numeriseur-lambda/live/numeriseur/handler.py) | Le routage par chemin (dont le `/photos/` qui était perdu), le portage Pillow du pipeline ImageMagick, et le mode de rejeu avec son filtre par compte. |
| [`live/numeriseur/build.sh`](https://github.com/ludorl82/aws-iac-public/blob/article/numeriseur-lambda/live/numeriseur/build.sh) | Trois lignes de `chmod` et le commentaire qui explique pourquoi elles décident de l'empreinte du paquet. |
| [`scripts/google-oauth-setup.py`](https://github.com/ludorl82/aws-iac-public/blob/article/numeriseur-lambda/scripts/google-oauth-setup.py) | La portée `drive.file`, le piège des sept jours, et l'option `--inspect` née de l'histoire des dossiers en double. |
| [`numeriseur/deployment.yaml`](https://github.com/ludorl82/k3s-iac-public/blob/article/numeriseur-lambda/numeriseur/deployment.yaml) | Le pod sans état, le `fsGroup`, et les sondes avec le commentaire sur `enable_rest_api`. |
| [`numeriseur/configmap.yaml`](https://github.com/ludorl82/k3s-iac-public/blob/article/numeriseur-lambda/numeriseur/configmap.yaml) | Le rendu en bash pur et les gardes qui refusent de démarrer plutôt que de mentir. |
