...
>
> -   **Le point de départ** : une carte achetée pour la machine de jeu en libère une autre. Le noeud GPU du cluster passe de deux à trois cartes.
> -   **La vérification qui change tout** : un balayage de tous les pods (runtime class et demandes de ressources) ne ramène **qu'un seul** pod GPU dans le cluster, épinglé par `nodeSelector` sur un autre noeud. « De la capacité GPU pour Kubernetes » n'avait donc aucun consommateur.
...
> -   **Où la carte a fini** : à servir la recherche sémantique du site, d'abord dans le service de l'hôte par facilité, puis **dans un pod**, pour que la carte réservée à Kubernetes ait un vrai consommateur. Le tunnel routait `/api/embed` vers le pod **par chemin**, donc pas de nouveau nom d'hôte à exposer ni à garder. *(Cette réservation a été défaite, refaite, puis défaite pour de bon le 15 septembre, quand le cluster s'est trouvé un noeud GPU ailleurs. Les trois mises à jour datées plus bas racontent chaque retournement et sa raison. Aujourd'hui : les trois cartes au modèle de conversation, le modèle d'embeddings de retour sur l'hôte.)*
...
Avant de déplacer quoi que ce soit, on a balayé le cluster au complet, tous les pods, en cherchant à la fois la `runtimeClass` NVIDIA et les demandes de ressources GPU. Le résultat tient sur une ligne : **un seul pod**, le NVR qui surveille la caméra, épinglé par `nodeSelector` sur le noeud de transcodage, qui a sa propre carte.
...
Une fois la troisième carte en place, le service de modèles de langage de l'hôte s'est étalé dessus tout seul. Ce n'est pas un problème en soi. On a mesuré, et l'étalement sur trois cartes donne exactement le même débit que sur deux. Mais ça veut dire que le premier pod GPU du cluster se serait battu avec lui pour la mémoire vidéo. Rien, dans cette machine, ne réserve une carte à qui que ce soit.
...
Le pipeline était à moitié local. Le modèle de langage, je l'avais rapatrié exprès, parce qu'un labo maison dont l'assistant répond depuis le centre de données de quelqu'un d'autre, ça manque un peu le point. Mais chaque phrase que je disais partait quand même chez un fournisseur infonuagique pour être transcrite. Un service de reconnaissance vocale sur GPU, dans le cluster, fermait cette moitié-là.

Déployé, mesuré. Sur un banc de douze sondes, des phrases générées en synthèse vocale et converties en audio brut, le modèle local a fait **10 sur 12**, contre 9 sur 12 au service infonuagique. Latence équivalente. J'ai basculé.
...
Le robot conversationnel de ce site connaissait l'**index** de mes articles, soit un titre, une date et une description tronquée, sans jamais avoir accès à leur contenu. Le corpus fait presque un mégaoctet de texte : il n'entrera jamais dans une fenêtre de contexte. La réponse à ça, c'est une recherche sémantique, et une recherche sémantique a besoin d'un modèle d'embeddings.

Premier réflexe : le mettre dans le service de modèles de l'hôte, à côté du modèle de conversation. C'était le chemin le plus court, parce que le Worker du site atteint déjà ce service par un chemin authentifié qui existe. Ça marchait, et j'ai failli m'arrêter là.
...
**Le modèle d'embeddings est parti dans un pod**, sur la carte réservée. Pas de `hostNetwork`, parce que l'hôte sert déjà son propre modèle sur le même port, et un pod en réseau d'hôte serait entré en collision avec lui. Un simple Service interne suffit : le build du site tourne sur un runner dans le cluster, et le Worker, lui, passe par le tunnel.
...
> **Mise à jour du 13 septembre 2026.** Ce jour est arrivé le lendemain. Un [banc d'essai sur le dessin d'architecture](/blog/le-banc-des-menteurs/) a montré qu'un modèle dense de 27 milliards de paramètres, de la génération suivante, fait le travail que le modèle résident ne sait pas faire, et qu'il lui faut les trois cartes pour tenir une fenêtre de 65 000 jetons. La réservation a donc été défaite : les trois cartes sont retournées au service de l'hôte, le modèle d'embeddings y est revenu, la règle de tunnel par chemin a disparu et le pod avec elle. La capacité GPU du cluster ira sur une autre machine, avec une autre carte. La leçon du paragraphe suivant tient toujours : la réservation avait un consommateur, c'est ce qui a permis de la défaire proprement plutôt que de la découvrir vide.
>
> **Et le soir même.** Les deux raisons de défaire sont tombées dans la journée : le modèle de 27 milliards lit ses prompts trop lentement pour servir, et l'autre carte ira finalement dans la machine de jeu, à la place de la GTX 960. La réservation a donc été **refaite** : deux cartes pour le modèle de conversation, la troisième pour Kubernetes, le pod d'embeddings dessus. Trois retournements en deux jours, chacun pour une raison mesurée. Je les laisse tous écrits ici plutôt que de faire comme si la configuration finale avait été évidente.
>
> **Mise à jour du 15 septembre 2026, et cette fois c'est fini.** La réservation est défaite pour de bon, et la raison ne peut plus se renverser : **le cluster a maintenant son propre noeud GPU ailleurs**, une machine virtuelle dédiée avec ses cartes à elle. Garder une carte réservée ici réservait donc la même capacité deux fois. Les trois cartes sont retournées au modèle de conversation, 36 gigaoctets, et le pod n'existe plus.
>
> **Le modèle d'embeddings, lui, n'a pas suivi le mandat vers le nouveau noeud**, et c'est la partie que je n'avais pas vue venir. Ce noeud-là s'éteint quand quelqu'un démarre une partie sur la machine de jeu. C'est voulu, six coeurs ne s'étirent pas. Or le site vectorise **la question du visiteur à chaque requête** : l'index des articles est bâti à la publication, mais comparer une question à cet index veut dire la transformer en direct. La recherche sémantique serait donc tombée le temps d'une partie. Et en silence, parce que le point de santé que surveille mon moniteur rapporte que le service a ses identifiants, pas qu'il retrouve quelque chose. Le modèle est revenu vivre dans le service de l'hôte, 664 mégaoctets à côté de 20 gigaoctets.
>
> **La leçon du paragraphe suivant tient toujours, et elle a même mieux vieilli que je pensais.** La réservation avait un consommateur réel, et c'est exactement ce qui a permis de la défaire proprement plutôt que de la découvrir vide : on savait quoi déménager, où, et ce que ça coûtait. Quatre retournements en quatre jours, chacun pour une raison mesurée. Je les laisse tous écrits ici plutôt que de faire comme si la configuration finale avait été évidente.
