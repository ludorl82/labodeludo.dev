...
> -   **Le verdict** : retour au service infonuagique pour la voix, et retrait du pod. La carte a fini par servir — mais dans un service de l'hôte, pas dans Kubernetes.
> -   **La leçon** : un GPU dans un cluster est une *capacité*, pas une *charge de travail*. Et un banc d'essai qui ne reproduit pas la panne ne mesure rien, même quand il affiche 10/12.
...
## La carte a trouvé un emploi — ailleurs que dans Kubernetes
...
Ce modèle vit maintenant sur la troisième carte. Sauf qu'il ne tourne pas dans Kubernetes : il tourne dans le service de modèles de l'hôte, à côté du modèle de conversation, parce que c'est là que le Worker du site l'atteint déjà par un chemin authentifié qui existait. Ajouter un pod aurait voulu dire un nouveau nom d'hôte public, une nouvelle règle d'accès et une nouvelle politique de pare-feu — pour rendre exactement le même service.

L'épingle sur deux cartes a donc sauté, et le modèle de conversation s'étale de nouveau sur les trois. Ce qui ne coûte rien : c'est mesuré.
