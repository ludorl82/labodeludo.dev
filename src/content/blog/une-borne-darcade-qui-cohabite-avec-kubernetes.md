...
La nouvelle maison du jeu, c'est une vieille tour de jeu qui traînait — appelons-le **l'hyperviseur**. Assez de muscle pour faire tourner des jeux récents, et surtout deux emplacements de carte graphique.

L'idée : ne pas installer un système de jeu directement sur l'hyperviseur, mais le découper en **deux machines virtuelles**, une par joueur de la maison. Chacune reçoit sa propre carte graphique, passée directement dans la VM — pas d'émulation, la vraie carte, avec toute sa puissance. C'est ça, le passthrough : le système hôte lâche complètement le GPU (`vfio-pci`), et la machine virtuelle le récupère comme s'il était branché dans son propre châssis.
...
Petit bonus de la même famille, moins grave : une fois l'hyperviseur en marche, un vrai moniteur était resté branché dans la carte graphique passée à la VM. GNOME a poliment mis le bureau sur cet écran-là, et la console de secours (l'écran émulé) n'affichait plus que le fond d'écran. Deux sorties vidéo, le bureau part sur la mauvaise. On force les deux à se copier, et l'image revient.
...
Sous le capot, ces interrupteurs ne font rien de magique : ils ouvrent une connexion SSH vers l'hyperviseur avec une **clé verrouillée sur une seule commande**. Cette clé n'a le droit que de dire `start`, `stop` ou `status` sur les deux postes, et rien d'autre. Même si quelqu'un la volait, il ne pourrait que démarrer et arrêter des bornes d'arcade. C'est la même discipline que partout ailleurs dans le labo : une porte, une seule chose derrière.
